...
def lsa(tfidf):
    ...
    print("LSA Reduced Matrix Shape: ")
    print reduced_matrix.shape
...
def svm_training(label, cv_index, matrix, kernel):
    ...
    for j in range(10):
        ...
        clf = svm.SVC(kernel=kernel, C=10, gamma=0.01)
        """
        #SVM Classification
        """
    ...
    print "====================Cross Validation of SVM Complete=============================="
...
def random_forests_training(label, cv_index, matrix, max):
    ...
    for j in range(10):
        ...
        clf = RandomForestClassifier(n_estimators=350, min_samples_split=2, bootstrap=True, random_state=1,
    ...
    print "====================Cross Validation of Random Forests for estimators = "+ str(max)+" Complete=============================="
    # print "Highest accuracy is " + str(max_score)
...
def neural_networks_training(label, cv_index, matrix, alpha):
    # Start 10-fold Cross Validation:
    score_array = []
    max_score = 0.0
    ...
    for j in range (10):
        ...
        clf = MLPClassifier(solver='adam', alpha=alpha, hidden_layer_sizes=(450, 675), random_state=1)
        print "Training Dataset Shape: "
        print datasets[0].shape
...
tfidf = bow(corpus)
lsa_reduced_matrix = lsa(tfidf)

#gamma = 0.01 best, c=100
...
#Random Forests Training + LSA
for num in max:
    rf_score[num] = np.mean(random_forests_training(label, cv_index, lsa_reduced_matrix, num))


#Neural Networks Training + LSA
#for num in alpha:
 #   nn_score[num] = np.mean(neural_networks_training(label, cv_index, lsa_reduced_matrix,num))

print " "
#for num in kernel:
   # print "SVM Score for gamma = "+ str(num) +": " + str(svm_score[num])
for num in max:
    print "Random Forests Score for estimators = " + str(num) + ": " + str(rf_score[num])
#for num in alpha:
 #   print "Neural Networks Score for alpha = " + str(num) + ": "+ str(np.mean(nn_score))
